...
- **Краевые пятна старения** (коричнево-рыжая «сепия» по краям листа) ложно ловятся
  красным порогом как разломы. Гард `DROP_BORDER_TOUCHING` убирает объекты, касающиеся
  края кадра (реальная геология лежит внутри рамки и до края не доходит), а триаж по
  краевому шуму (`EDGE_NOISE_FRAC`) помечает такие карты `low_confidence`, даже если
  объектов много. На прогоне по 94 реальным картам это убрало ~1800 ложных линий и
  честно подсветило трудные сепия-сканы.
