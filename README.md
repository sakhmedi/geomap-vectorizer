...
|:---:|:---:|
| ![Оригинал](examples/example_original.jpg) | ![Вектора](examples/example_overlay.jpg) |
...
- **Легенда и рамка карты** иногда попадают в результат (цветные образцы в легенде).
  Есть опциональная автообрезка до рамки карты (`CROP_TO_MAP_BORDER` в `config.py`),
  но на мятых исторических картах рамка детектится ненадёжно, поэтому по умолчанию ВЫКЛ.
- Длинные разломы могут разбиваться на сегменты; частично сшиваются «мостом»
  (`BRIDGE_KERNEL_SIZE` в `config.py`), но не всегда полностью.
...
- Более надёжная обрезка области карты (текущая опция отключена по умолчанию).
- Осевые линии разломов через скелетизацию (`scikit-image`) — чище вектор, чем контур-«петля».
